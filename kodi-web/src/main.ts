...
document.querySelector<HTMLDivElement>('#app')!.innerHTML = `<div class="app-shell">
  <aside class="sidebar">
    ...
    <nav class="primary-nav" aria-label="Primary navigation">
      ...
      <button class="nav-item" data-view="Live TV">${icon('tv')}<span>Live TV</span></button>
      <button class="nav-item" data-view="Add-ons">${icon('plus')}<span>Add-ons</span><b class="new-badge">NEW</b></button>
      <button class="nav-item" data-view="Games">${icon('play')}<span>Games</span></button>
    ...
    <nav class="secondary-nav" aria-label="Library navigation">
      <button class="nav-item" data-view="ROMs">${icon('play')}<span>ROMs</span><b>2.4k</b></button>
  ...
  <main class="main-content">
    <header class="topbar"><button class="mobile-menu" aria-label="Open menu">${icon('more')}</button><div class="breadcrumb"><span>Library</span><strong>/</strong><span id="view-label">Home</span></div><label class="search-box">${icon('search')}<input id="search" placeholder="Search movies, shows, games..." type="search"><kbd>⌘ K</kbd></label><input id="local-file" type="file" accept="video/*,.mkv,.avi,.mov,.mp4,.webm" hidden><button class="load-local" id="load-local" type="button">${icon('plus')} Load local file</button><button class="avatar" aria-label="Profile">JD</button></header>
    <section class="hero" aria-label="Online Google Drive video"><iframe class="hero-drive" title="Online Google Drive video" src="${googleDrivePreview}" allow="autoplay; fullscreen" allowfullscreen></iframe><div class="hero-shade"></div><div class="hero-content"><span class="eyebrow">Online video</span><h1>Google Drive</h1><p class="hero-meta">Shared video  <i></i>  Drive playback</p><p class="hero-description">Your online video is ready to play from Google Drive. Use the player controls to start sound and fullscreen playback.</p><div class="hero-actions"><a class="primary-action" id="play-featured" href="https://drive.google.com/file/d/1mHolwVL7b3ndLrGn1dmDjrOIG7d3JgDo/view?usp=sharing" target="_blank" rel="noopener noreferrer">${icon('play')} Open video</a><button class="video-action" id="watch-featured">Play in dialog</button><button class="circle-action" aria-label="Add to favorites">${icon('plus')}</button></div></div><div class="hero-index"><strong>01</strong><span>/ 01</span></div></section>
    <section class="quick-access"><div class="section-heading"><div><span class="section-kicker">Quick access</span><h2>Everything in one place</h2></div></div><div class="quick-grid"><button class="quick-tile blue">${icon('film')}<span>Movies<small>245 titles</small></span></button><button class="quick-tile purple">${icon('tv')}<span>TV Shows<small>312 shows</small></span></button><button class="quick-tile gold">${icon('play')}<span>Games<small>128 games</small></span></button><button class="quick-tile green">${icon('play')}<span>ROMs<small>2,464 ROMs</small></span></button><button class="quick-tile cyan">${icon('tv')}<span>Live TV<small>17 channels</small></span></button><button class="quick-tile blue">${icon('music')}<span>Music<small>1,860 albums</small></span></button></div></section>
