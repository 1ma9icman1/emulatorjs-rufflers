...
export function isNesRomName(name?: string): boolean {
  const normalized = (name || "").toLowerCase();
  return normalized.endsWith(".nes");
}

// Raw JSON games typed as RomItem
export const ALL_DRIVE_ROMS: RomItem[] = (masterGames as any[])
  .filter((g) => isNesRomName(g.rawName || g.title))
  .map((g) => ({
