...
export class NesEngine {
  ...
  public async loadRom(data: ArrayBuffer | Uint8Array) {
    try {
      ...
      if (romBuffer.length < 16) {
        throw new Error("ROM is too small to be a valid NES file.");
      }

      const header = String.fromCharCode(...romBuffer.subarray(0, 4));
      if (header !== "NES\u001a") {
        throw new Error("Selected file is not a valid .nes ROM (missing iNES header).");
      }
